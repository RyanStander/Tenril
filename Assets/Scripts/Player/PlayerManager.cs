...
namespace Player
{
    ...
    public class PlayerManager : CharacterManager
    {
        ...
        public PlayerSpellcastingManager playerSpellcastingManager;
        public PlayerQuickslotManager playerQuickslotManager;
        ...
        private void HandleStates()
        {
            switch (playerState)
            {
                ...
                case PlayerState.Dead:
                    ...
                    if (Math.Abs(restartTimeStamp) < 0.001f)
        ...
        internal void HideDisplayObject(bool hideLeftHandObject = true)
        {
        ...
        private void OnEquipWeapon(EventData eventData)
        {
            if (eventData is EquipWeapon equipWeapon)
            {
                ...
                var oldWeapon = equipWeapon.isPrimaryWeapon ? playerInventory.primaryWeapon : playerInventory.secondaryWeapon;
        ...
        private void OnUseItem(EventData eventData)
        {
            ...
            {
                if (!(useItem.item is ConsumableItem quickslotItem)) return;
        ...
        private void OnLoadPlayerCharacterData(EventData eventData)
        {
            ...
            {
                var playerData = loadPlayerCharacterData.playerData;

                playerStats.SetPlayerStats(playerData);

                var position = new Vector3(playerData.position[0], playerData.position[1], playerData.position[2]);
                var rotation = Quaternion.Euler(playerData.rotation[0], playerData.rotation[1], playerData.rotation[2]);
        ...
        private void OnAwardPlayerXP(EventData eventData)
        {
            ...
            {
                //Calculate if the player will receive any level ups
        ...
        private void OnPlayerKeybindsUpdates(EventData eventData)
        {
            ...
            {
                var spellSlotUiManager = FindObjectOfType<SpellSlotUIManager>();
                if (spellSlotUiManager == null) return;
                var bindingPath = CharacterUtilityManager.GetBindingPath(inputHandler.GetInputActions().CharacterControls.SpellcastingMode, inputHandler.activeInputDevice);
                //Spellcasting Mode icon
                var spellcastingModeSprite = CharacterUtilityManager.FindKeybindIcon(inputHandler.deviceDisplayConfigurator, inputHandler.activeInputDevice, bindingPath);

                //Spell slots
                var spellSlotKeybindSprites = new Sprite[8];
        ...
        {
            if (!(eventData is PlayerChangedInputDevice)) return;
            var spellSlotUiManager = FindObjectOfType<SpellSlotUIManager>();
            var bindingPath = CharacterUtilityManager.GetBindingPath(inputHandler.GetInputActions().CharacterControls.SpellcastingMode, inputHandler.activeInputDevice);
